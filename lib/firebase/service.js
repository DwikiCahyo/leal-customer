...
export async function postClaimVoucher(
  db,
  voucherId,
  userId,
  voucherPin,
  ...
  const userFollowedRef = doc(db, `user/${userId}/Followed`, merchantId);

  const claimVoucher = await updateDoc(userRef, {
    claimed_vouchers: arrayUnion(voucherId),
  });

  const updateStarOwned = await updateDoc(userFollowedRef, {
    starsOwned: starOwned - voucherPin,
  });
